...
function fetchScanData(tickers = [], type = '', columns = []) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!data.startsWith('{'))
            throw new Error('Wrong screener or symbol');
        try {
...
module.exports = {
    ...
    */
    getScreener(exchange) {
        ...
        if (['NASDAQ', 'NYSE', 'NYSE ARCA', 'OTC'].includes(e))
            return 'america';
        if (['ASX'].includes(e))
            return 'australia';
        if (['TSX', 'TSXV', 'CSE', 'NEO'].includes(e))
            return 'canada';
        if (['EGX'].includes(e))
            return 'egypt';
        if (['FWB', 'SWB', 'XETR'].includes(e))
            return 'germany';
        if (['BSE', 'NSE'].includes(e))
            return 'india';
        if (['TASE'].includes(e))
            return 'israel';
        if (['MIL', 'MILSEDEX'].includes(e))
            return 'italy';
        if (['LUXSE'].includes(e))
            return 'luxembourg';
        if (['NEWCONNECT'].includes(e))
            return 'poland';
        if (['NGM'].includes(e))
            return 'sweden';
        if (['BIST'].includes(e))
            return 'turkey';
        if (['LSE', 'LSIN'].includes(e))
            return 'uk';
        if (['HNX'].includes(e))
            return 'vietnam';
    ...
    getTA(screener, id) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const cols = ['1', '5', '15', '60', '240', '1D', '1W', '1M']
                ...
                .flat();
            const rs = yield fetchScanData([id], screener, cols);
    ...
    searchIndicator(search = '') {
        return __awaiter(this, void 0, void 0, function* () {
            if (!builtInIndicList.length) {
                yield Promise.all(['standard', 'candlestick', 'fundamental'].map((type) => __awaiter(this, void 0, void 0, function* () {
                    builtInIndicList.push(...(yield request({
            ...
            return [
                ...builtInIndicList.filter((i) => (norm(i.scriptName).includes(norm(search))
                    || norm(i.extra.shortDescription).includes(norm(search)))).map((ind) => ({
                    id: ind.scriptIdPart,
                    version: ind.version,
                    name: ind.scriptName,
                    author: {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            Object.keys(data.result.metaInfo.styles).forEach((plotId) => {
                ...
                if (titles.includes(plotTitle)) {
                    let i = 2;
                    while (titles.includes(`${plotTitle}_${i}`))
                        i += 1;
            ...
            data.result.metaInfo.plots.forEach((plot) => {
                ...
                if (!plot.target)
                    return;
                plots[plot.id] = `${(_a = plots[plot.target]) !== null && _a !== void 0 ? _a : plot.target}_${plot.type}`;
            });
    ...
    loginUser(username, password, remember = true, UA = 'TWAPI/3.0') {
        ...
        return __awaiter(this, void 0, void 0, function* () {
            ...
            const { data, cookies } = yield request({
                method: 'POST',
                host: 'www.tradingview.com',
            ...
            const session = ((_a = cookie.match(/sessionid=(.*?);/)) !== null && _a !== void 0 ? _a : [])[1];
            return {
                id: data.user.id,
     ...
     * @returns {Promise<User>} Token
     */
    getUser(session, location = 'https://www.tradingview.com/') {
        return __awaiter(this, void 0, void 0, function* () {
                ...
                https.get(location, {
                    ...
                    res.on('end', () => __awaiter(this, void 0, void 0, function* () {
                        if (res.headers.location && location !== res.headers.location) {
                            cb(yield module.exports.getUser(session, res.headers.location));
                            return;
                        }
                        if (rs.includes('auth_token')) {
                            ...
                            err(new Error('Wrong or expired sessionid'));
                    }));
                    res.on('error', err);
                }).end();
            });
     ...
     */
    getDrawings(layout, symbol = '', credentials = {}, chartID = 1) {
        return __awaiter(this, void 0, void 0, function* () {
            const chartToken = yield module.exports.getChartToken(layout, credentials);
            const creds = credentials.id && credentials.session;
            const session = creds ? credentials.session : null;
            const { data } = yield request({
                host: 'charts-storage.tradingview.com',
                path: `/charts-storage/layout/${layout}/sources?chart_id=${chartID}&jwt=${chartToken}${symbol ? `&symbol=${symbol}` : ''}`,
                headers: { cookie: session ? `sessionid=${session}` : '' },
            });
            if (!data.payload)
                throw new Error('Wrong layout, user credentials, or chart id.');
            return Object.values(data.payload.sources || {}).map((drawing) => (Object.assign(Object.assign({}, drawing), drawing.state)));
        });
    },
};
